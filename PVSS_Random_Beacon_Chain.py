...
class PVSS_chain:
    ...
    def __init__(self, n, t):
        ...
        for i in range(0, self.n):
            self.participants.append(PVSS.PVSS(self.n, self.t))
            self.participants[i].Generate(i + 1)
        self.pub_key = sorted({i + 1: int(PVSS.PVSS.pub_key[i]) for i in range(self.n)}.items(), key=lambda d: d[1],
                              reverse=False)
        #print(self.pub_key)
    ...
    def __next__(self):
        ...
        hr = hashlib.sha256()
        Li = PVSS_chain.leader_selection(self)
        ...
        if not need_re:
            ...
            for i in range(self.t):
                self.dec_shares[i] = int(PVSS.PVSS.member_secrets[Li][party[i]])
            self.index += 1
            self.blocks.append(
                {
                    "Index": self.index,
